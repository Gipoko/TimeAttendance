...
class rloan extends DB\SQL\Mapper {
	...
	public function add($unsanitizeddata) {
		...
		$data['rloan_created']=$this->getCurrentDateTime();
		$data['rloan_updated']=$this->getCurrentDateTime();
		$this->copyFrom($data);
		$this->insert();
		return $this->GET('rloan_id');
	}

	public function edit($id, $unsanitizeddata) {
		$data=$this->sanitizeInput($unsanitizeddata, $this->allowed_fields);
		$this->load(array('rloan_id=?',$id));
		$this->copyFrom('POST');
		$data['rloan_updated']=$this->getCurrentDateTime();
	...
	public function list() {
		$sql= "SELECT * FROM rloan WHERE rloan_is_active = 'Active' ORDER BY rloan_id";
		return $this->db->exec($sql);
	}
}