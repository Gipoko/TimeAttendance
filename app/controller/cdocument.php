...
public function index(){
    ...
    $oUserMenu = new menu_skote();
			...
			$this->f3->set('content','/page/setup/payroll/document.html');
		...
		$oDocument = new rdocument($this->db);

		$result = $oDocument->countRecords();
...
if($searchValue != ''){
$searchQuery = " and (
	rdocument_type like '%".$searchValue."%' or
	rdocument_desc like '%".$searchValue."%'

						) ";
}

		## Total number of record with filtering
		$oRecords = $oDocument->countRecordsWithFilter($searchQuery);
		$totalRecordwithFilter = $oRecords[0]['totalFilteredRecords'];;

		## Fetch records
		$oRecords = $oDocument->getRecords($searchQuery, $columnName, $columnSortOrder, $row, $rowperpage);

		$data = [];

		for($i=0;$i<count($oRecords);$i++) {
			$id = $oRecords[$i]['rdocument_id'];
			$action =  '';
			// if ((int)$id > 1) {
			...
			$data[] = array(
                "rdocument_id"=>$oRecords[$i]['rdocument_id'],
                "rdocument_type"=>$oRecords[$i]['rdocument_type'],
                "rdocument_desc"=>$oRecords[$i]['rdocument_desc'],
                "rdocument_status"=>$oRecords[$i]['rdocument_status'],
...
public function add() {
	...
	//since we are adding new Document need to delete the Document_id which is auto increment
	//in the db.
	unset($_POST['rdocument_id']);
	...
	$error = false;
	try {
		$lastId = $oDocument->add($this->f3->GET('POST'));
	...
	if ($error) {
		...
		echo json_encode(array('status'=>true, 'msgContent'=>"Document added successfully", 'msgTitle'=>'Success', 'icon'=>'success'));
...
public function Edit() {
	$id = $this->f3->GET('POST.rdocument_id');
	$oDocument = new rdocument($this->db);

	$errMsg = '';
	$error = false;
	try {
		$lastId = $oDocument->edit($id, $this->f3->GET('POST'));
	} catch (PDOException $e) {
		$errMsg = $e->getMessage();
		$error = true;
	}
	...
	if ($error) {
		...
		echo json_encode(array('status'=>true, 'msgContent'=>"Document updated successfully", 'msgTitle'=>'Success', 'icon'=>'success'));
...
public function Delete() {
	...
	if ($error) {
		...
		echo json_encode(array('status'=>true, 'msgContent'=>"Document deleted successfully", 'msgTitle'=>'Success', 'icon'=>'success'));
