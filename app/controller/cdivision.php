...
public function index(){
    ...
    $oUserMenu = new menu_skote();
			...
			$this->f3->set('content','/page/setup/office/division.html');
		...
		if($searchValue != ''){
		$searchQuery = " and (
			rdivision_code like '%".$searchValue."%' or
			rdivision_name like '%".$searchValue."%'

								) ";
		}

		$oDivision = new rdivision($this->db);
		...
		for($i=0;$i<count($oRecords);$i++) {
			...
			$data[] = array(
				"rdivision_id"=>$oRecords[$i]['rdivision_id'],
				"rdivision_code"=>$oRecords[$i]['rdivision_code'],
				"rdivision_name"=>$oRecords[$i]['rdivision_name'],
...
public function add() {
	...
	if ($error) {
		...
		echo json_encode(array('status'=>true, 'msgContent'=>"Division added successfully", 'msgTitle'=>'Success', 'icon'=>'success'));
...
public function Edit() {
	$id = $this->f3->GET('POST.rdivision_id');
	$oDivision = new rdivision($this->db);
	...
	if ($error) {
		...
		echo json_encode(array('status'=>true, 'msgContent'=>"Division updated successfully", 'msgTitle'=>'Succcess', 'icon'=>'success'));
...
public function Delete() {
	...
	if ($error) {
		...
		echo json_encode(array('status'=>true, 'msgContent'=>"Division deleted successfully", 'msgTitle'=>'success', 'icon'=>'success'));
...
public function list() {
	$oList = new rDivision($this->db);
	$resDivision = $oList->list();
	$data = [];
	for($i=0;$i<count($resDivision);$i++) {
		$data[] = array('id'=>$resDivision[$i]['rdivision_id'], 'text'=>$resDivision[$i]['rdivision_name']);
	}
