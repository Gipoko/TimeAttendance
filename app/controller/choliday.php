...
public function index(){
    ...
    $oUserMenu = new menu_skote();
			...
			$this->f3->set('content','/page/setup/payroll/holiday.html');
		...
		if($searchValue != ''){
		$searchQuery = " and (
			rholiday_code like '%".$searchValue."%' or
			rholiday_desc like '%".$searchValue."%'

								) ";
		}

		$oHoliday = new rholiday($this->db);

		$result = $oHoliday->countRecords();
		...
		$oRecords = $oHoliday->countRecordsWithFilter($searchQuery);
		$totalRecordwithFilter = $oRecords[0]['totalFilteredRecords'];;

		## Fetch records
		$oRecords = $oHoliday->getRecords($searchQuery, $columnName, $columnSortOrder, $row, $rowperpage);

		$data = [];

		for($i=0;$i<count($oRecords);$i++) {
			$id = $oRecords[$i]['rholiday_id'];
			$action =  '';
			// if ((int)$id > 1) {
			...
			$data[] = array(
				"rholiday_id"=>$oRecords[$i]['rholiday_id'],
				"rholiday_code"=>$oRecords[$i]['rholiday_code'],
				"rholiday_desc"=>$oRecords[$i]['rholiday_desc'],
                "rholiday_date"=>$oRecords[$i]['rholiday_date'],
                "rholiday_status"=>$oRecords[$i]['rholiday_status'],
				"action"=>$action
			);
		}
...
public function add() {
	...
	//since we are adding new Holiday need to delete the Holiday_id which is auto increment
	//in the db.
	unset($_POST['rholiday_id']);
	...
	$error = false;
	try {
		$lastId = $oHoliday->add($this->f3->GET('POST'));
	...
	if ($error) {
		...
		echo json_encode(array('status'=>true, 'msgContent'=>"Holiday added successfully", 'msgTitle'=>'Success', 'icon'=>'success'));
...
public function Edit() {
	$id = $this->f3->GET('POST.rholiday_id');
	$oHoliday = new rholiday($this->db);

	$errMsg = '';
	$error = false;
	try {
		$lastId = $oHoliday->edit($id, $this->f3->GET('POST'));
	} catch (PDOException $e) {
		$errMsg = $e->getMessage();
		$error = true;
	}
	...
	if ($error) {
		...
		echo json_encode(array('status'=>true, 'msgContent'=>"Holiday updated successfully", 'msgTitle'=>'Success', 'icon'=>'success'));
...
public function Delete() {
	...
	if ($error) {
		...
		echo json_encode(array('status'=>true, 'msgContent'=>"Holiday deleted successfully", 'msgTitle'=>'Message', 'icon'=>'success'));
