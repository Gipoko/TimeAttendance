<?php
class cgradestep extends cau{

public function index(){
    if($this->f3->VERB=='GET') {
    $oUserMenu = new menu_skote();
			$menu = $oUserMenu->loadMenu();

			$this->f3->SET('sidebar', $menu);
			if($this->f3->GET('SESSION.user_group_id') === 5)
			{
				 $this->f3->set('content','page/unauthorized.html');
				 $this->f3->set('layout','page/unauth.html');
			}else{
			$this->f3->set('content','/page/setup/payroll/gradestep.html');
			$this->f3->set('layout','/page/layout.html');
			}
    } else {

		$draw = $this->f3->GET('POST.draw');
		$row = $this->f3->GET('POST.start');
		$rowperpage = $this->f3->GET('POST.length'); // Rows display per page

			$columnIndex = $this->f3->GET("POST.order[0]['column']"); // Column index
		//$columnIndex = $_POST['order'][0]['column']; // Column index
		//var_dump($columnIndex);
		$columnName = $this->f3->GET("POST.columns[$columnIndex]['data']"); // Column name


		$columnSortOrder = $this->f3->GET("POST.order[0]['dir']"); // asc or desc
		$searchValue = $this->f3->GET("POST.search['value']"); // Search value

		

		$oGradestep = new rgradestep($this->db);

		$result = $oGradestep->countRecords();

		$totalRecords = $result[0]['totalRecords'];

		## Search
$searchQuery = " ";
if($searchValue != ''){
$searchQuery = " and (
	rgradestep_gradeno like '%".$searchValue."%' or
	rgradestep_stepno like '%".$searchValue."%' or
	rgradestep_salary like '%".$searchValue."%' or
	rgradestep_effectivity like '%".$searchValue."%' 
						) ";
}

		## Total number of record with filtering
		$oRecords = $oGradestep->countRecordsWithFilter($searchQuery);
		$totalRecordwithFilter = $oRecords[0]['totalFilteredRecords'];;

		## Fetch records
		$oRecords = $oGradestep->getRecords($searchQuery, $columnName, $columnSortOrder, $row, $rowperpage);

		$data = [];

		for($i=0;$i<count($oRecords);$i++) {
			$id = $oRecords[$i]['rgradestep_id'];
			$action =  '';
			// if ((int)$id > 1) {
				
				$action .= '<div class="btn-group">';
				$action .= '	<button type="button" class="btn btn-info dropdown-toggle dropdown-toggle-split" data-bs-toggle="dropdown" aria-expanded="false">';
				$action .= '		<i class="dripicons-gear"></i>';
				$action .= '	</button>';
				$action .= '	<div class="dropdown-menu">';
				$action .= '		<a class="dropdown-item btnEdit" href="#" data-id="'.$id.'">Edit</a>';
				$action .= '		<a class="dropdown-item btnDelete" href="#" data-id="'.$id.'">Delete</a>';
				$action .= '	</div>';
				$action .= '</div>';
			// }
		

			$data[] = array(
                "rgradestep_id"=>$oRecords[$i]['rgradestep_id'],
                "rgradestep_gradeno"=>$oRecords[$i]['rgradestep_gradeno'],
                "rgradestep_stepno"=>$oRecords[$i]['rgradestep_stepno'],
                "rgradestep_salary"=>$oRecords[$i]['rgradestep_salary'],
                "rgradestep_effectivity"=>$oRecords[$i]['rgradestep_effectivity'],
                "rgradestep_is_active"=>$oRecords[$i]['rgradestep_is_active'],
				"action"=>$action
			);
		}

		## Response
		$response = array(
		"draw" => intval($draw),
		"iTotalRecords" => $totalRecords,
		"iTotalDisplayRecords" => $totalRecordwithFilter,
		"aaData" => $data
		);

		echo json_encode($response);
		die;
	}
}
    
public function add() {

	//since we are adding new Grade Step need to delete the Grade Step_id which is auto increment
	//in the db.
	unset($_POST['rgradestep_id']);

	$oGradestep = new rgradestep($this->db);

	$errMsg = '';
	$error = false;
	try {
		$lastId = $oGradestep->add($this->f3->GET('POST'));
	} catch (PDOException $e) {
		$errMsg = $e->getMessage();
		$error = true;
	}

	if ($error) {
		echo json_encode(array('status'=>false, 'msgContent'=>$errMsg, 'msgTitle'=>'Error', 'icon'=>'error'));

	} else {
		echo json_encode(array('status'=>true, 'msgContent'=>"Grade Step added successfully", 'msgTitle'=>'Success', 'icon'=>'success'));

	}
	die;
}

public function Edit() {
	$id = $this->f3->GET('POST.rgradestep_id');
	$oGradestep = new rgradestep($this->db);

	$errMsg = '';
	$error = false;
	try {
		$lastId = $oGradestep->edit($id, $this->f3->GET('POST'));
	} catch (PDOException $e) {
		$errMsg = $e->getMessage();
		$error = true;
	}

	if ($error) {
		echo json_encode(array('status'=>false, 'msgContent'=>$errMsg, 'msgTitle'=>'Error', 'icon'=>'error'));

	} else {
		echo json_encode(array('status'=>true, 'msgContent'=>"Grade Step updated successfully", 'msgTitle'=>'Success', 'icon'=>'success'));

	}
	die;
}

public function Delete() {
	$id = $this->f3->GET('POST.rgradestep_id');
	$oGradestep = new rgradestep($this->db);

	$errMsg = "";
	$error = false;
	try {
		$lastId = $oGradestep->delete($id);
	} catch (PDOException $e) {
		//There are active records in child table
		$errMsg = $e->getMessage();
		$error = true;
	}

	if ($error) {
		echo json_encode(array('status'=>false, 'msgContent'=>$errMsg, 'msgTitle'=>'Error', 'icon'=>'error'));

	} else {
		echo json_encode(array('status'=>true, 'msgContent'=>"Grade Step deleted successfully", 'msgTitle'=>'Success', 'icon'=>'success'));

	}
	die;
}

public function findGradeStep() {
	$grade = $this->f3->GET('POST.gradeno');
	$step = $this->f3->GET('POST.stepno');
	$id = $this->f3->GET('POST.id');

	if ($grade == '' || $step == '') {
		echo json_encode(array('rgradestep_salary'=>0));
		die;
	}
	$year = date('Y');
	$oList = new rGradeStep($this->db);
	$resGradeNo = $oList->findGradeStep($grade, $step, $year);

	echo json_encode($resGradeNo[0]);
	die;
}

public function findStep(){
	$grade = $this->f3->GET('POST.gradeno');
	$step = $this->f3->GET('POST.stepno');
	

	if ($grade == '' || $step == '') {
		echo json_encode(array('rgradestep_salary'=>0));
		die;
	}
	
	$oList = new rGradeStep($this->db);
	$resGradeNo = $oList->findStep($grade, $step);

	echo json_encode($resGradeNo[0]);
	die;
}

}
