...
public function index(){
    ...
    $oUserMenu = new menu_skote();
			...
			$this->f3->set('content','/page/setup/payroll/loan.html');
		...
		if($searchValue != ''){
		$searchQuery = " and (
			rloan_code like '%".$searchValue."%' or
			rloan_desc like '%".$searchValue."%'

								) ";
		}

		$oLoan = new rloan($this->db);

		$result = $oLoan->countRecords();
		...
		$oRecords = $oLoan->countRecordsWithFilter($searchQuery);
		$totalRecordwithFilter = $oRecords[0]['totalFilteredRecords'];;

		## Fetch records
		$oRecords = $oLoan->getRecords($searchQuery, $columnName, $columnSortOrder, $row, $rowperpage);

		$data = [];

		for($i=0;$i<count($oRecords);$i++) {
			$id = $oRecords[$i]['rloan_id'];
			$action =  '';
			// if ((int)$id > 1) {
			...
			$data[] = array(
				"rloan_id"=>$oRecords[$i]['rloan_id'],
				"rloan_code"=>$oRecords[$i]['rloan_code'],
				"rloan_desc"=>$oRecords[$i]['rloan_desc'],
				"rloan_type"=>$oRecords[$i]['rloan_type'],
				"action"=>$action
			);
		}
...
public function add() {
	...
	//since we are adding new Loan need to delete the Loan_id which is auto increment
	//in the db.
	unset($_POST['rloan_id']);
	...
	$error = false;
	try {
		$lastId = $oLoan->add($this->f3->GET('POST'));
	...
	if ($error) {
		...
		echo json_encode(array('status'=>true, 'msgContent'=>"Loan added successfully", 'msgTitle'=>'Success', 'icon'=>'success'));
...
public function Edit() {
	$id = $this->f3->GET('POST.rloan_id');
	$oLoan = new rloan($this->db);

	$errMsg = '';
	$error = false;
	try {
		$lastId = $oLoan->edit($id, $this->f3->GET('POST'));
	} catch (PDOException $e) {
		$errMsg = $e->getMessage();
		$error = true;
	}
	...
	if ($error) {
		...
		echo json_encode(array('status'=>true, 'msgContent'=>"Loan updated successfully", 'msgTitle'=>'Success', 'icon'=>'success'));
...
public function Delete() {
	...
	if ($error) {
		...
		echo json_encode(array('status'=>true, 'msgContent'=>"Loan deleted successfully", 'msgTitle'=>'Success', 'icon'=>'success'));
