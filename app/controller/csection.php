...
public function index(){
    ...
    $oUserMenu = new menu_skote();
			...
			$this->f3->set('content','/page/setup/office/section.html');
		...
		if($searchValue != ''){
		$searchQuery = " and (
			rsection_code like '%".$searchValue."%' or
			rsection_name like '%".$searchValue."%'

								) ";
		}

		$oSection = new rsection($this->db);

		$result = $oSection->countRecords();
		...
		$oRecords = $oSection->countRecordsWithFilter($searchQuery);
		$totalRecordwithFilter = $oRecords[0]['totalFilteredRecords'];;

		## Fetch records
		$oRecords = $oSection->getRecords($searchQuery, $columnName, $columnSortOrder, $row, $rowperpage);

		$data = [];

		for($i=0;$i<count($oRecords);$i++) {
			$id = $oRecords[$i]['rsection_id'];
			$action =  '';
			// if ((int)$id > 1) {
			...
			$data[] = array(
				"rsection_id"=>$oRecords[$i]['rsection_id'],
				"rsection_code"=>$oRecords[$i]['rsection_code'],
				"rsection_name"=>$oRecords[$i]['rsection_name'],
...
public function add() {
	...
	$error = false;
	try {
		$lastId = $oSection->add($this->f3->GET('POST'));
	...
	if ($error) {
		...
		echo json_encode(array('status'=>true, 'msgContent'=>"Section added successfully", 'msgTitle'=>'Success', 'icon'=>'success'));
...
public function Edit() {
	$id = $this->f3->GET('POST.rsection_id');
	$oSection = new rsection($this->db);

	$errMsg = '';
	$error = false;
	try {
		$lastId = $oSection->edit($id, $this->f3->GET('POST'));
	} catch (PDOException $e) {
		$errMsg = $e->getMessage();
		$error = true;
	}
	...
	if ($error) {
		...
		echo json_encode(array('status'=>true, 'msgContent'=>"Section updated successfully", 'msgTitle'=>'Success', 'icon'=>'success'));
...
public function Delete() {
	...
	if ($error) {
		...
		echo json_encode(array('status'=>true, 'msgContent'=>"Section deleted successfully", 'msgTitle'=>'Success', 'icon'=>'success'));
